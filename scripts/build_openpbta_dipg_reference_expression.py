#!/usr/bin/env python
"""Build the checksum-pinned OpenPBTA v23 H3-altered DIPG reference."""
...
from oncoref.expression_source_adapters import build_openpbta_dipg_source_matrices
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--polya-expression-path", type=Path, default=None)
    parser.add_argument("--stranded-expression-path", type=Path, default=None)
    ...
    cache = args.cache_dir or (
        Path.home() / ".cache" / "oncoref" / "expression" / "openpbta-v23-dipg-h3k27"
    )
    result = build_openpbta_dipg_source_matrices(
        cache_dir=cache,
        output_dir=args.output_dir,
        polya_expression_path=args.polya_expression_path,
        stranded_expression_path=args.stranded_expression_path,
